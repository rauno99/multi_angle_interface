...
public:
  ...
  void publishLineStrip(const std::vector<geometry_msgs::Point>& points);
  void steeringAngleCallback(const sensor_msgs::Joy::ConstPtr& joy_msg);
...
private:
  ...
  void createSphere();
  void initializeLineStrip(visualization_msgs::Marker& marker, const std::string& ns, int id, float r, float g, float b);
